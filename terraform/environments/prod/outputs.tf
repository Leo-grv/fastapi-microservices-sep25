...
output "database_url" {
  description = "URL de connexion PostgreSQL (sans mot de passe)"
  value       = "postgresql://${var.rds_master_username}:***@${aws_db_instance.postgresql.endpoint}/${var.rds_database_name}"
  sensitive   = true
}
...
output "db_address" {
  description = "Adresse de la base de données (sans port)"
  value       = aws_db_instance.postgresql.address
}
